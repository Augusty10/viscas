...
import { X, Calendar, MapPin, AlignLeft, Users, Loader2 } from "lucide-react";
import { useCalendarStore } from "@/hooks/useCalendar";
import { updateEvent } from "@/lib/calendar";

export default function EditEventDialog() {
  const { isEditEventOpen, setEditEventOpen, editingEvent, triggerRefresh } = useCalendarStore();
  ...
  useEffect(() => {
    if (isEditEventOpen && editingEvent) {
      ...
      setStartDate(formatDateTimeLocal(editingEvent.start));
      setEndDate(formatDateTimeLocal(editingEvent.end));

      // Prefill attendees
      if (editingEvent.attendees && editingEvent.attendees.length > 0) {
        const emails = editingEvent.attendees.map((a: any) => a.email || a).join(", ");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!editingEvent?.id) {
      setError("No event is selected to update.");
      return;
    }
      ...
      <div className="w-full max-w-lg overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-6 py-4">
          <div className="flex items-center gap-2 text-sky-600">
            ...
            <h2 className="text-lg font-bold text-slate-800">Edit Calendar Event</h2>
          </div>
          <button
            onClick={() => setEditEventOpen(false)}
            className="rounded-full p-1.5 text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition cursor-pointer"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
